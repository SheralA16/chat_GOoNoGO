...
// UniversalMessage puede ser un mensaje de texto o imagen
...
func (m *UniversalMessage) ToJSON() ([]byte, error) {
...
func NewImageMessage(username, content, imageData, imageName string, imageSize int64) *UniversalMessage {
	// Detectar tipo MIME basado en la extensión
	mimeType := mime.TypeByExtension(filepath.Ext(imageName))
	if mimeType == "" {
		// Intentar detectar desde los datos base64
